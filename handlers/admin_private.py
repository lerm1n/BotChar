...
@admin_router.message(F.text == 'Библиотека')
async def admin_features(message: types.Message, session: AsyncSession):
    categories = await orm_get_categories(session)
    btns = {category.name : f'category_{category.id}' for category in categories}
    await message.answer("Выберите тему", reply_markup=get_callback_btns(btns=btns))
...
async def starring_at_product(callback: types.CallbackQuery, session: AsyncSession):
    ...
    await callback.message.answer("ОК, вот список Данных ")
...
class AddProduct(StatesGroup):
    ...
    texts = {
        "AddProduct:name": "Введите Заголовок заново:",
        "AddProduct:description": "Введите описание заново:",
        "AddProduct:category": "Выберите Тему  заново ⬆️",
...
async def change_product_callback(
    ...
    await callback.message.answer(
        "Введите название персонажа/механики", reply_markup=types.ReplyKeyboardRemove()
    )
    await state.set_state(AddProduct.name)


# Становимся в состояние ожидания ввода name
@admin_router.message(StateFilter(None), F.text == "Добавить персонажа/механику")
async def add_product(message: types.Message, state: FSMContext):
    await message.answer(
        "Введите название заголовка", reply_markup=types.ReplyKeyboardRemove()
    )
    await state.set_state(AddProduct.name)
...
async def add_name(message: types.Message, state: FSMContext):
    ...
    await message.answer("Введите описание персонажа/механики")
    await state.set_state(AddProduct.description)

# Хендлер для отлова некорректных вводов для состояния name
@admin_router.message(AddProduct.name)
async def add_name2(message: types.Message, state: FSMContext):
    await message.answer("Вы ввели не допустимые данные, введите текст заголовка персонажа/механики")
...
async def add_description2(message: types.Message, state: FSMContext):
    await message.answer("Вы ввели не допустимые данные, введите текст описания персонажа/механики")
...
async def category_choice(callback: types.CallbackQuery, state: FSMContext , session: AsyncSession):
    if int(callback.data) in [category.id for category in await orm_get_categories(session)]:
        ...
        await callback.message.answer('Выберите тему из кнопок.')
        await callback.answer()

#Ловим любые некорректные действия, кроме нажатия на кнопку выбора категории
@admin_router.message(AddProduct.category)
async def category_choice2(message: types.Message, state: FSMContext):
    await message.answer("'Выберите тему из кнопок.'") 
...
async def add_image(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await message.answer("персонаж/механика добавлен/изменен", reply_markup=ADMIN_KB)
        await state.clear()
...
async def add_image2(message: types.Message, state: FSMContext):
    await message.answer("Отправьте желаемое изображение ")
